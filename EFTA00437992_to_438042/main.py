...
def get_filenames(root):
    ...
    for directory in os.listdir(root):
        if directory.startswith("EFTA"):
            for fname in os.listdir(os.path.join(root, directory)):
                if fname.endswith(".png"):
                    ret.append(os.path.join(root, directory, fname))
    return ret
...
def find_rects(img, expand=(0, 0, 0, 0)):
    rects = []
    q = [(0, 0)]
    seen = set()
    seen.add(q[0])
...
def process_pages(filenames, text_area, white_thresh=0.95, use_for_rect_detection=()):
    ...
    print("Finding rects...")
    imgs_for_rects = imgs if len(use_for_rect_detection) == 0 else [pygame.image.load(filename).subsurface(text_area)
                                                                    for filename in use_for_rect_detection]
    xaxis, yaxis = multismear(imgs_for_rects)
...
if __name__ == "__main__":
    ...
    output = process_pages(get_filenames("input"), [33, 40, 750, 983], use_for_rect_detection=(
    ...
    while True:
        ...
        if mode == 'normal':
            ...
            screen.blit(output["imgs"][page_idx], (0, 0))
            for r in output["rects"]:
                pygame.draw.rect(screen, 'red', r, width=1)
